...
script.Parent.Touched:connect(function(hit)
	if hit.Name == "SSBall" then
		...
		if hit:WaitForChild("PlayerOnBall").Value == nil then
			...
			local touchedAgain = false
			local conn = hit.Touched:Connect(function(hitactor)
				if game.Players:GetPlayerFromCharacter(hitactor.Parent) then
					touchedAgain = true
				end
			end)
		...
		for i, child in pairs(script.Parent.Parent.Parent:GetChildren()) do
			if child ~= script.Parent.Parent then
				local scorer = hit.LastShot.Value
				
				
				local num = child.Score.Value
				num = num + 1
				child.Score.Value = num
				Srevent:Fire(scorer,'Score')
