...
--			end
			...
			weld:Destroy()
			return	vector * power,nPos,ball,false,forcedir	
...
--			local newTime = time() - temptime
			...
			return	(absVel*0.95),nPos,ball,false,forcedir
		end --Ballend
	end --WeldEnd	
end --FunctionEnd
...
--			end
			...
			return	newVel,nPos,ball,false,forcedir
		end --Ballend
	end --WeldEnd
end --FunctionEnd
	...
	if weld then
		...
		if ball then
			...
			if vector.y < -0.82 or vector.y > 0.82 then
				...
				newVel = Vector3.new(vector.x, 0.21, vector.z) * 3 * (multiplier + addition)
			end
